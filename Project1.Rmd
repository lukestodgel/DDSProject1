...
beerIPAandAle = combined_df %>% filter(grepl("IPA",Style) | grepl("Ale",Style))
beerIPAandAle = droplevels(beerIPAandAle,exclude = !beerIPAandAle)
beerIPAandAle <- na.omit(beerIPAandAle)
...
for(j in 1:iterations)
{
  ...
  for(i in 1:numks)
  {
    CM = confusionMatrix(table(beerIPAandAle[,6],knn.cv(beerIPAandAle[,c(4,5)],beerIPAandAle[,6],k = i)))
...
#did not find average specificity and sensitivity because they were getting NA's and causing errors.



#knn.cv on IBU IPAs, comment on how accurate and sensitive and specific it was
#repeat for ABV
...
#Canned beer vs Bottled beer - preference?
#are people buying becasue of taste? or alcohol content? or price?
